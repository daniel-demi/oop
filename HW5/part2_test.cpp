...
//    assert(Stream<int>::of(vector).map<Cell<int>>([](const int* a) { return new Cell<int>(*a); }).distinct().count() == 0);
...
//    assert(Stream<int>::of(vector2).min() == nullptr);
//    assert(Stream<int>::of(vector2).max() == nullptr);
...
//    assert(Stream<int>::of(vector2).findFirst([](const int* x) -> bool {return *x < 0;}) == nullptr);
    Stream<int>::of(vector2).forEach([](const int* i) -> void {
        std::cout << *i << std::endl;
    });
    return 0;
}
